 ...
 * /api/open/** 为对外系统集成接口（WMS / TMS / OMS 业务单据推送），改用 X-Api-Key 校验。
 */
@Component
    ...
    public boolean preHandle(HttpServletRequest req, HttpServletResponse res, Object handler) throws IOException {
        ...
        if (req.getRequestURI().startsWith(OPEN_PREFIX)) {
            ...
            if (openApiKey == null || openApiKey.isEmpty() || given == null
                    || !MessageDigest.isEqual(given.getBytes(StandardCharsets.UTF_8), openApiKey.getBytes(StandardCharsets.UTF_8))) {
